    ...
    if (g_led_display.timer_flag) {
        ...
        switch (g_led_display.mode) {
            ...
            case LED_MODE_RUNNING:
                /* 跑马灯模式（任务3） */
                process_running_light();
                break;
                
            case LED_MODE_FLASH_LOW_DIGITS:
                /* 闪烁低位模式（任务3） */
                process_flash_low_digits();
                break;
                
            case LED_MODE_CLEAR_SEQUENCE:
                /* 清除序列模式（任务4） */
...
void start_running_light(void)
{
    ...
    /* 设置跑马灯模式 */
    g_led_display.mode = LED_MODE_RUNNING;
    g_led_display.current_led = 0;           /* 从LED1开始 */
    g_led_display.running_direction = 1;     /* 正向 */
    g_led_display.flash_state = 0;
    
    /* 激活显示并启动1秒定时器 */
    g_led_display.display_active = 1;
    timer_start(TIMER_1SEC);
}

/* 启动闪烁低两位模式（任务3：非整数倍时显示） */
void start_flash_low_digits(void)
{
    ...
    /* 设置闪烁低位模式 */
    g_led_display.mode = LED_MODE_FLASH_LOW_DIGITS;
    g_led_display.current_led = 0;           /* 从个位开始 */
    g_led_display.flash_count = 0;
    g_led_display.flash_state = 0;
    
    /* 设置个位数字的闪烁次数 */
    g_led_display.target_flashes = g_system.digits[0]; /* 个位 */
    if (g_led_display.target_flashes == 0) {
        g_led_display.target_flashes = 10;
    }
...
void start_clear_sequence(void)
{
    ...
    /* 设置清除序列模式 */
    g_led_display.mode = LED_MODE_CLEAR_SEQUENCE;
    g_led_display.clear_flash_count = 0;     /* 清除闪烁计数 */
    g_led_display.flash_state = 0;           /* 开始为灭状态 */
    
    /* 激活显示并启动0.5秒定时器 */
    g_led_display.display_active = 1;
    timer_start(TIMER_05SEC);
}

/* 处理跑马灯显示（任务3） */
void process_running_light(void)
{
    if (!g_led_display.display_active) return;
    ...
    /* 计算下一个位置（双向跑马灯：0→1→2→3→2→1→0→1...） */
    if (g_led_display.running_direction == 1) {
        /* 正向 */
        g_led_display.current_led++;
        if (g_led_display.current_led >= MAX_LEDS) {
            g_led_display.current_led = MAX_LEDS - 2;
            g_led_display.running_direction = 0; /* 改为反向 */
        }
    } else {
        /* 反向 */
        if (g_led_display.current_led == 0) {
            g_led_display.current_led = 1;
            g_led_display.running_direction = 1; /* 改为正向 */
        } else {
            g_led_display.current_led--;
        }
    }
    ...
    if (g_led_display.flash_state == 0) {
        ...
        if (g_led_display.flash_count >= g_led_display.target_flashes) {
            ...
            if (g_led_display.current_led == 0) {
                /* 从个位切换到十位 */
                g_led_display.current_led = 1;
                g_led_display.target_flashes = g_system.digits[1]; /* 十位 */
            } else {
                /* 从十位切换到个位 */
                g_led_display.current_led = 0;
                g_led_display.target_flashes = g_system.digits[0]; /* 个位 */
...
void process_clear_sequence(void)
{
    ...
    if (g_led_display.flash_state == 0) {
        ...
        /* 完成了一次闪烁 */
        g_led_display.clear_flash_count++;
        
        /* 检查是否完成了3次闪烁 */
        if (g_led_display.clear_flash_count >= 3) {
            /* 完成清除序列，进入空闲状态 */
            g_led_display.display_active = 0;
            g_led_display.mode = LED_MODE_IDLE;
            g_led_display.clear_flash_count = 0;
            g_led_display.flash_state = 0;
            g_led_display.k3_hold_detected = 0;
