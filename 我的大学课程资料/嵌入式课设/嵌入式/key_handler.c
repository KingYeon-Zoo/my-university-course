...
void key_state_machine(uint8_t key_id)
{
    ...
    if (key_id == 2 && k3_pressed && !g_system.key_prev[2]) {
        /* K3按下，首先检测是否与K4组合 */
        detect_k3_k4_combination();
        /* 如果不是组合按键，触发数字显示 */
        if (!g_led_display.k3_hold_detected) {
    ...
    /* 检测K4按下事件（任务3触发条件） */
    if (key_id == 3 && g_system.key_pressed[3] && !g_system.key_prev[3]) {
        /* K4按下，处理比较判断功能 */
        handle_k4_press();
        return;
    }
...
void handle_k4_press(void)
{
    ...
    /* 计算日期各位数字之和 */
    date_sum = calculate_date_sum();
    
    /* 判断是否为整数倍 */
    if (is_multiple(current_number, date_sum)) {
        /* 是整数倍，显示双向跑马灯 */
        start_running_light();
    } else {
        /* 不是整数倍，闪烁低两位数字 */
        start_flash_low_digits();
    }
}

/* 计算固定日期各位数字之和：2025-6-27 */
uint32_t calculate_date_sum(void)
{
    /* 2025-6-27 → 2+0+2+5+6+2+7 = 24 */
    return FIXED_DATE_SUM;
}
...
{
    uint8_t k3_stable, k4_stable;
    
    /* 获取当前按键状态 */
    k3_stable = get_key_state(2); /* K3 */
    k4_stable = get_key_state(3); /* K4 */
    
    /* 如果K3保持按下且K4也被按下，触发清除序列 */
    if (k3_stable && k4_stable) {
        g_led_display.k3_hold_detected = 1;
