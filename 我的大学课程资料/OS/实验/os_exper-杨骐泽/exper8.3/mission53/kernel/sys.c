...
int sys_table_mapping()
{
	...
	calc_mem();	// æ˜¾ç¤ºå†…å­˜ç©ºé—²é¡µé¢æ•°ã€‚
	
	// é¦–å…ˆæ‰“å°è¾“å‡ºé¡µç›®å½•ä¿¡æ¯ã€‚æ ¼å¼ä¸ºï¼šPage Directory(é¡µç›®å½•çš„ç‰©ç†é¡µæ¡†å· | æ‰€åœ¨çº¿æ€§åœ°å€)
	fprintk(1,"Page Directory(PFN:0x0 | LA:0x00000000)\n");
	
	// ç¬¬ä¸€å±‚å¾ªçŽ¯ï¼ŒéåŽ†é¡µç›®å½•ä¸­çš„æ‰€æœ‰ PDE
	for(index_of_dir = 0; index_of_dir < 1024; index_of_dir++)
	{
		entry = ((unsigned long*)page_dir_base)[index_of_dir];
		
		if(!(entry & 1))	/* è·³è¿‡æ— æ•ˆçš„ PDE */
			continue;
		
		// è¾“å‡º PDE ä¿¡æ¯,æ ¼å¼å¦‚ä¸‹ï¼š  
		// PDE: ä¸‹æ ‡ -> Page Table(é¡µè¡¨çš„ç‰©ç†é¡µæ¡†å· | æ‰€åœ¨çš„çº¿æ€§åœ°å€)
		fprintk(1,"\tPDE: 0x%X -> Page Table(PFN:0x%X | LA:0x%08X)\n", index_of_dir,  entry >> 12, 0xFFFFF000 & entry);

		/* é¡µç›®å½•é¡¹çš„é«˜20ä½å³ä¸ºé¡µè¡¨çš„ç‰©ç†åœ°å€ï¼Œé¡µè¡¨çš„ç‰©ç†åœ°å€å³ä¸ºé¡µè¡¨çš„é€»è¾‘åœ°å€ */
		page_table_base = 0xFFFFF000 & entry;
		
		/* ç¬¬äºŒå±‚å¾ªçŽ¯ï¼ŒéåŽ†é¡µè¡¨ä¸­çš„æ‰€æœ‰ PTE */
		for(index_of_table = 0; index_of_table < 1024; index_of_table++)
		{
			entry = ((unsigned long*)page_table_base)[index_of_table];
			
			if(!(entry & 1))	/* è·³è¿‡æ— æ•ˆçš„ PTE */
				continue;	
			
			// è¾“å‡º PTE ä¿¡æ¯ï¼Œæ ¼å¼å¦‚ä¸‹ï¼š
			// PTE: ä¸‹æ ‡ -> Physical Page(ç‰©ç†é¡µæ¡†å· | æ‰€åœ¨çš„çº¿æ€§åœ°å€)
