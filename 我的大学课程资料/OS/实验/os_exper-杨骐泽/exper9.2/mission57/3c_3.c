...
void resetUserSpace(void);
void runAutoTests(void);
static void runAutoScenario(int freeLargeFirst);
...
Space allocBoundTag(Space * pav,int size){
	...
	if (best->size - size > ALLOC_MIN_SIZE){//从高位截取p,不破坏指针间的关系.
		Space remain = best;
		remain->size -= size;
		Space foot = FOOT_LOC(remain);
		foot->upLink = remain;
		foot->tag = 0;
		Space alloc = foot + 1;
		alloc->size = size;
		foot = FOOT_LOC(alloc);
		alloc->tag = foot->tag = 1;
		foot->upLink = alloc;
		p = alloc;
	}
	else{//分配后剩余空间小于 ALLOC_MIN_SIZE 
		if (best->nextLink == best){//只剩下一个空间了，清空指针
			*pav = NULL;
		}
		else{//直接分配 best->size个空间出去
			best->preLink->nextLink = best->nextLink;
			best->nextLink->preLink = best->preLink;
			if (*pav == best){
				*pav = best->nextLink;
			}
		}
		Space foot = FOOT_LOC(best);
		foot->tag =best->tag = 1;
		p = best;
	}
	userSpace[usCount] = p;
...
void reclaimBoundTag(Space * pav,Space sp){
	...
	if(*pav != NULL)
	{
		Space leftFoot = sp - 1;//左侧边界
		pTag = leftFoot->tag;
		if (pTag == 0){
			pre = leftFoot->upLink;//只有左邻为空闲块时才需要访问头部
		}
	...
	if ((*pav != NULL && pTag == 1 && nTag == 1) || *pav == NULL){//前后都被占用，直接插入在表头.
		...
		if (next->preLink == next && next->nextLink == next){//only one free block in list
			sp->preLink = sp;
			sp->nextLink = sp;
		}else{
...
static void runAutoScenario(int freeLargeFirst){
	...
	resetUserSpace();
	initSpace(&freeSpace,&pav);
	Space block880 = allocBoundTag(&pav, 880);
	Space block120 = allocBoundTag(&pav, 120);
	if (block880 == NULL || block120 == NULL){
		printf("[AUTO] allocation failed\n");
		if (freeSpace){
			free(freeSpace);
		}
		return;
	...
	if (freeLargeFirst){
		printf("[AUTO] free order: 880 -> 120 (right merge)\n");
		reclaimBoundTag(&pav, block880);
		reclaimBoundTag(&pav, block120);
	}else{
		printf("[AUTO] free order: 120 -> 880 (left merge)\n");
		reclaimBoundTag(&pav, block120);
		reclaimBoundTag(&pav, block880);
	}
	if (pav && pav->size == MAX_SIZE){
		printf("[AUTO] PASS: free size restored to %d\n", pav->size);
	}else if (pav){
		printf("[AUTO] WARN: free size = %d (expected %d)\n", pav->size, MAX_SIZE);
	}else{
		printf("[AUTO] WARN: free list is empty\n");
	}
	if (freeSpace){
		free(freeSpace);
	}
...
void runAutoTests(void){
	...
	printf("[AUTO] Scenario 1: release 880 then 120\n");
	runAutoScenario(1);
	printf("[AUTO] Scenario 2: release 120 then 880\n");
	runAutoScenario(0);
}
