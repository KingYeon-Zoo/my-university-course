  ...
  <div class="app-container">
    ...
    <el-form :inline="true" :model="formInline" class="demo-form-inline">
      ...
      <el-form-item label="课程选择">
        <el-select v-model="searchCourseId" placeholder="选择课程" clearable @change="handleSearchCourseChange" style="width: 150px">
          <el-option
            v-for="item in searchCoursesList"
            :key="item.id"
      ...
      <el-form-item label="章节选择">
        <el-select v-model="searchChapterId" placeholder="选择章节" clearable :disabled="!searchCourseId" style="width: 150px">
          <el-option
            v-for="item in searchChaptersList"
            :key="item.id"
      ...
      <el-form-item label="题型选择">
        <el-select v-model="searchQuestionTypeId" placeholder="选择题型" clearable :disabled="!searchCourseId" style="width: 150px">
          <el-option
            v-for="item in searchQuestionTypesList"
            :key="item.id"
            :label="item.typeName"
            :value="item.id"
          />
      ...
      <el-form-item label="题库(旧)">
        <repo-select
          v-model="selectedRepoSingleSearch"
          @change="handleRepoChangeSingle"
        />
      </el-form-item>
      <el-form-item label="类型(旧)">
        <el-select v-model="selValue" placeholder="请选择" style="width: 120px">
          <el-option
    ...
    >
      ...
      <el-table-column prop="chapterName" label="所属章节" align="center" />
      <el-table-column prop="extractCount" label="被抽次数" align="center" width="100" />
...
export default {
  ...
  data() {
    return {
      ...
      searchChapterId: undefined,
      searchQuestionTypeId: undefined,
      searchCoursesList: [],
      searchChaptersList: [],
      searchQuestionTypesList: [],
  ...
  methods: {
    ...
    handleSearchCourseChange(courseId) {
      this.searchChapterId = undefined
      this.searchQuestionTypeId = undefined
      this.searchChaptersList = []
      this.searchQuestionTypesList = []
      if (courseId) {
        listChapter(courseId).then(res => {
          if (res.code) {
            this.searchChaptersList = res.data
          }
        })
        getTypesByCourse(courseId).then(res => {
          if (res.code) {
            this.searchQuestionTypesList = res.data
          }
        })
      }
    },
    ...
    // 分页查询
    async getQuPage(pageNum = this.pageNum, pageSize = this.pageSize, content = null, repoId = null, type = null, courseId = this.searchCourseId, chapterId = this.searchChapterId, questionTypeId = this.searchQuestionTypeId) {
      const params = {
        pageNum: pageNum,
        pageSize: pageSize,
        content: content,
